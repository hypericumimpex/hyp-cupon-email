...
function ywces_register_exporter( $exporters ) {
	$exporters['ywces-accept'] = array(
		'exporter_friendly_name' => __( 'Coupon Email System', 'yith-woocommerce-coupon-email-system' ),
...
function ywces_exporter( $email_address ) {
	...
	if ( $user && get_user_meta( $user->ID, 'ywces_receive_coupons', true ) == 'yes' ) {
		...
		$data_to_export[] = array(
			...
			'group_label' => __( 'Coupon Email System Status', 'yith-woocommerce-coupon-email-system' ),
			'item_id'     => "accept-0",
			'data'        => array(
				array(
					'name'  => __( 'Acceptance', 'yith-woocommerce-coupon-email-system' ),
					'value' => __( 'This customer has given consent to receive coupons', 'yith-woocommerce-coupon-email-system' ),
				)
			),
		);
...
function ywces_register_eraser( $erasers ) {
	$erasers['ywces-accept'] = array(
		'eraser_friendly_name' => __( 'Coupon Email System', 'yith-woocommerce-coupon-email-system' ),
